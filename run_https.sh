...
if [ ! -f "certs/dev-cert.pem" ] || [ ! -f "certs/dev-key.pem" ]; then
  echo "[setup] No hay certificados HTTPS. Generando certs locales..."
  "$PYTHON_BIN" scripts/setup_https.py
fi

echo "[run] Arrancando SKU Vision en HTTPS..."
exec "$PYTHON_BIN" scripts/dev_up_https.py "$@"
